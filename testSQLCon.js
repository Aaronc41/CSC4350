  ...
  pool.getConnection((err, con)=>{
    ...
    con.query(sql, (err, res, cols)=>{
      ...
      for(var i=0; i<res.length; i++){
        table +='<tr><td>'+ (i+1) +'</td><td>'+ res[i].aurora_ID +'</td><td>'+ res[i].first_name +'</td>'
        table +='<td>'+ res[i].last_name +'</td><td>'+ res[i].email +'</td></tr>';
      }
      table ='<table border="1"><tr><th>Nr.</th><th>aurora_ID</th><th>first_name</th><th>last_name</th><th>email</th></tr>'+ table +'</table>';
...
let sql = "select aurora_ID, first_name, last_name, email, password, participant_status, department from users";
